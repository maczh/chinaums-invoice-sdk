...
type QueryInvoiceStatusResponse struct {
    ...
    InvoiceMaterial      string `json:"invoiceMaterial,omitempty"`        // 发票材质: PAPER/ELECTRONIC
    InvoiceType          string `json:"invoiceType,omitempty"`            // 发票类型: PLAIN/VAT
    InvoiceNo            string `json:"invoiceNo,omitempty"`              // 发票号码(1-20字符)
    InvoiceCode          string `json:"invoiceCode,omitempty"`            // 发票代码(1-20字符)
    CheckCode            string `json:"checkCode,omitempty"`              // 校验码(1-32字符)
    CipherCode           string `json:"cipherCode,omitempty"`             // 密码区(1-256字符)
    IssueDate            string `json:"issueDate,omitempty"`              // 开票日期(yyyy-MM-dd HH:mm:ss格式)
    ReverseInvoiceNo     string `json:"reverseInvoiceNo,omitempty"`       // 红票的发票号码(1-20字符)
    ReverseInvoiceCode   string `json:"reverseInvoiceCode,omitempty"`     // 红票的发票代码(1-20字符)
    ReverseCheckCode     string `json:"reverseCheckCode,omitempty"`       // 红票的校验码(1-32字符)
    ReverseCipherCode    string `json:"reverseCipherCode,omitempty"`      // 红票的密码区(1-256字符)
    ReverseDate          string `json:"reverseDate,omitempty"`            // 红冲日期(yyyy-MM-dd HH:mm:ss格式)
    ...
    TerminalName         string `json:"terminalName,omitempty"`           // 银商终端名称(1-128字符)
    ...
    SellerName           string `json:"sellerName,omitempty"`             // 卖方名称(1-128字符)
    ...
    DeductionAmount      float64 `json:"deductionAmount"`        // 扣除额(单位为元),保留两位小数
    TotalPriceIncludingTax float64 `json:"totalPriceIncludingTax"` // 含税总金额(单位为元),保留两位小数
    TotalTax             float64 `json:"totalTax"`               // 税额(单位为元),保留两位小数
    TotalPrice           float64 `json:"totalPrice"`             // 不含税总金额(单位为元),保留两位小数
    ...
    MerWxAppID           string `json:"merWxAppId,omitempty"`             // 商户微信AppId(1-32字符)
    MerWxOrderID         string `json:"merWxOrderId,omitempty"`           // 商户微信OrderId(1-64字符)
    IssueQRCode          string `json:"issueQrCode,omitempty"`            // 开票二维码(1-128字符)
    ...
    SubList              []any `json:"subList,omitempty"`                // 子订单信息(JsonArray格式的字符串)
}
...
type RedReverseInvoiceResponse struct {
    ...
    ReverseDate        string `json:"reverseDate,omitempty"`         // 红冲日期(yyyy-MM-dd HH:mm:ss格式)
    DeviceNo           string `json:"deviceNo,omitempty"`            // 机器编号(1-32字符)
    QRCodeID           string `json:"qrCodeId,omitempty"`            // 二维码唯一id(40字符),电子发票必填
    StoreID            string `json:"storeId,omitempty"`             // 门店编号(1-32字符)
    StoreName          string `json:"storeName,omitempty"`           // 门店名称(1-128字符)
    MerchantName       string `json:"merchantName,omitempty"`        // 银商商户名称(1-128字符)
    MerchantID         string `json:"merchantId,omitempty"`          // 银商商户号(15字符)
    ...
    Drawer             string `json:"drawer,omitempty"`              // 开票人(1-8字符)
    Remark             string `json:"remark,omitempty"`              // 备注(1-128字符)
    TaxMethod          string `json:"taxMethod,omitempty"`           // 征税方式: NORMAL/DEDUCTION
...
type InvalidInvoiceResponse struct {
    ...
    CipherCode         string `json:"cipherCode,omitempty"`          // 密码区(1-256字符)
    IssueDate          string `json:"issueDate,omitempty"`           // 开票日期(yyyy-MM-dd HH:mm:ss格式)
    InvalidDate        string `json:"invalidDate,omitempty"`         // 作废日期(yyyy-MM-dd HH:mm:ss格式)
    DeviceNo           string `json:"deviceNo,omitempty"`            // 机器编号(1-32字符)
    QRCodeID           string `json:"qrCodeId,omitempty"`            // 二维码唯一id(40字符)
    StoreID            string `json:"storeId,omitempty"`             // 门店编号(1-32字符)
    StoreName          string `json:"storeName,omitempty"`           // 门店名称(1-128字符)
    ...
    Remark             string `json:"remark,omitempty"`              // 备注(1-128字符)
    TaxMethod          string `json:"taxMethod,omitempty"`           // 征税方式: NORMAL/DEDUCTION
    DeductionAmount    float64 `json:"deductionAmount,omitempty"`     // 扣除额(单位为元)
    TotalPriceIncludingTax float64 `json:"totalPriceIncludingTax,omitempty"` // 含税总金额(单位为元)
    TotalTax               float64 `json:"totalTax,omitempty"`               // 税额(单位为元)
    TotalPrice             float64 `json:"totalPrice,omitempty"`             // 不含税总金额(单位为元)
    GoodsDetail            string `json:"goodsDetail,omitempty"`            // 商品明细(1-1024字符)
    NotifyMobileNo     string `json:"notifyMobileNo,omitempty"`      // 消费者手机号(1-16字符)
    NotifyEmail        string `json:"notifyEmail,omitempty"`         // 消费者邮箱(1-64字符)
    NotifyUrl          string `json:"notifyUrl,omitempty"`           // 商户通知地址(1-128字符)
    ...
    PdfUrl             string `json:"pdfUrl,omitempty"`              // PDF下载链接(1-128字符)
    PdfPreviewUrl      string `json:"pdfPreviewUrl,omitempty"`       // PDF预览链接(1-128字符)
    ErrMsg             string `json:"errMsg,omitempty"`              // 作废结果信息(1-256字符)
    SubList            []any `json:"subList,omitempty"`             // 子订单信息(JsonArray格式的字符串)
}

...
type DirectIssueRequest struct {
	...
	BuyerBrevityCode      string `json:"buyerBrevityCode"`      // 买方简码(1-32字符)
	BuyerGroupBrevityCode string `json:"buyerGroupBrevityCode"` // 买方集团简码(1-32字符)
	...
	NotifyMobileNo        string `json:"notifyMobileNo"`        // 消费者手机号，用于短信通知电票(1-16字符)
	NotifyEmail           string `json:"notifyEmail"`           // 消费者邮箱，用于邮件通知电票(1-64字符)
	NotifyUrl             string `json:"notifyUrl"`             // 商户通知地址，用于开票结果通知(1-256字符)
	MerWxAppId            string `json:"merWxAppId"`            // 商户微信AppId，用于微信插卡(1-32字符)
	MerWxOrderId          string `json:"merWxOrderId"`          // 商户微信OrderId，用于微信插卡(1-64字符)
	AlipayUserId          string `json:"alipayUserId"`          // 用户支付宝userId，用于支付宝插卡(1-16字符)
	StoreId               string `json:"storeId"`               // 门店号(1-32字符)
	...
	OperateId             string `json:"operateId"`             // 营运ID(1-18字符)
	VehicleNo             string `json:"vehicleNo"`             // 车牌号(1-10字符)
	OutTradeNo            string `json:"outTradeNo"`            // 三方自定义的订单号(1-64字符)
	NotFaceRemark         string `json:"notFaceRemark"`         // 非票面备注(1-256字符)
	SpecialInvoiceFlag    string `json:"specialInvoiceFlag"`    // 特殊票种标识(2字符)
	InvoiceSpecialInfo    string `json:"invoiceSpecialInfo"`    // 特殊票种信息(JSON字符串，最大15000字符)
	ConfirmIssue          string `json:"confirmIssue"`          // 全电成品油单价过低是否确认开具标识(2字符)
	InvoiceBalanceInfo    string `json:"invoiceBalanceInfo"`    // 数电发票差额扣除额凭证明细(JSON数组)
}
...
type DirectIssueResponse struct {
	...
	OfdUrl                 string        `json:"ofdUrl"`                 // OFD下载链接(1-128字符)
	OfdPreviewUrl          string        `json:"ofdPreviewUrl"`          // OFD预览链接(1-128字符)
	XmlUrl                 string        `json:"xmlUrl"`                 // XML文件地址(1-128字符)
	XmlPreviewUrl          string        `json:"xmlPreviewUrl"`          // XML预览链接(1-128字符)
...
type GoodsDetail struct {
	Index                 string  `json:"index"`                      // 行号（从1开始）
